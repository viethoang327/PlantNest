...
namespace PlantNestApp.Repository
{
	public interface IProduct : IBaseRepository<Product>
	{
		...
		Task<List<ProductMinify>> GetproductBycategoriesAsync(int id);
	...
	public class ProductRepository : BaseRepository<Product>, IProduct
	{
		...
		public async Task<List<ProductMinify>> GetproductBycategoriesAsync(int id)
		{
			var query = _db.categoryInProducts.Include(r => r.Product).Include(r => r.Category).Where(r => r.Product.isDeleted != true && r.CategoryID==id);
			var result = from q in query
						 group q by new
						 {
							 q.Product.Name,
